...
export class DoctorAppointments implements OnInit {
  ...
  loadFilterOptions(): void {
    ...
    this.appointmentService.getFilterDates(docId).subscribe({
      ...
      error: err => console.error('[DoctorAppointments] Error fetching dates', err)
    });
    this.appointmentService.getFilterPatients(docId).subscribe({
      ...
      error: err => console.error('[DoctorAppointments] Error fetching patients', err)
    });
  }
  ...
  loadUsers(): void {
    this.authService.getAllUsers().subscribe({
      next: (users) => {
        users.forEach(u => this.usersMap.set(u.userId, u));
        this.cdr.detectChanges();
      },
      error: (err) => console.error('[DoctorAppointments] Error fetching users', err)
    });
  }
  ...
  viewPatientProfile(patientId: number, event: Event): void {
    ...
    this.patientProfileService.getProfileByUserId(patientId).subscribe({
      next: (profile) => this.router.navigate([`/patient/profile/${profile.id}`]),
