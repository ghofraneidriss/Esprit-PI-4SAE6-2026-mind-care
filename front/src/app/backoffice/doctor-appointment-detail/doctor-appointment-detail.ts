...
export class DoctorAppointmentDetail implements OnInit {
      ...
      next: (apt) => {
        ...
        this.authService.getAllUsers().subscribe({
          next: (users) => {
            ...
            const user = users.find(u => u.userId == apt.patientId || Number(u.userId) === Number(apt.patientId));
          ...
          error: (err) => {
            ...
            this.authService.getUserById(apt.patientId).subscribe({
              next: (user) => {
                this.patientUser = user;
                this.profileService.getProfileByEmail(user.email).subscribe({
                  next: (p) => { this.patientProfile = p; this.cdr.detectChanges(); },
                  error: () => this.cdr.detectChanges()
                });
              },
              error: (err2) => {
  ...
  saveDateUpdate(): void {
    ...
    // Rétablissement des contrôles de saisie (Demande utilisateur)
